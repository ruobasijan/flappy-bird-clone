...
class BaseScene extends Phaser.Scene {
    ...
    protected fontOptions: Phaser.Types.GameObjects.Text.TextStyle = { fontSize: `${this.fontSize}px`, color: '#000' };
    ...
    create() {
        ...
        if (this.config.canGoBack) {
            const backButton = this.add.image(this.config.width - 2, this.config.height - 2, 'back').setOrigin(1).setScale(2).setInteractive();
    ...
    createMenu(menu: {text: string, }, setupMenuEvents) {
